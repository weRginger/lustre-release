                                                ...
                                                struct ldlm_extent *new_ex)
{
	...
	printk("Inside ldlm_extent_internal_policy_granted!\n");
	printk("__u64 req_start is %llu; __u64 req_end is %llu\n", req_start, req_end);
	...
	printk("__u64 new_ex->start is %llu; __u64 new_ex->end is %llu\n", new_ex->start, new_ex->end);
			     ...
			     struct list_head *work_list)
{
	...
	*err = ELDLM_OK;
	...
	printk("ldlm_process_extent_lock(): lock->l_cc = %d\n", lock->l_cc);
